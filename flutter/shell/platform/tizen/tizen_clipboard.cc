...
void TizenClipboard::SetData(const std::string& data) {
  ...
  data_source_ =
      wl_data_device_manager_create_data_source(data_device_manager_);
  if (!data_source_) {
  ...
  static const wl_data_source_listener kSourceListener = {
      HandleDataSourceTarget,      HandleDataSourceSend,
      HandleDataSourceCancelled,   HandleDataSourceDndDropPerformed,
      HandleDataSourceDndFinished, HandleDataSourceAction,
  };
...
bool TizenClipboard::InitializeDataDevice() {
  ...
  data_device_ =
      wl_data_device_manager_get_data_device(data_device_manager_, seat_);
  if (!data_device_) {
    return false;
  }

  static const wl_data_device_listener kDeviceListener = {
      HandleDataOffer,  HandleDataEnter, HandleDataLeave,
      HandleDataMotion, HandleDataDrop,  HandleSelection,
  };
