...
void TextInputChannel::HandleMethodCall(
  ...
  if (method.compare(kShowMethod) == 0) {
    ...
    // A new client starts a fresh editing session; drop any composition
    // still pending in the IMF context from the previous client.
    input_method_context_->ResetInputMethodContext();
    ...
    // Reset the IMF context only when the state actually changes. The
    // framework echoes the state back during composition (e.g. through
    // controller listeners), and an unconditional reset would cancel the
    // ongoing composition.
